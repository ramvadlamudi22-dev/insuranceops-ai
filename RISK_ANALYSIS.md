...
| TR-004 | Poison-pill `Task` fails on every worker and loops forever | Medium | Medium | Medium | Bounded retries per Step (default 3 attempts, configurable in code, never unbounded); a `Task` that exhausts retries moves to the dead-letter list; a worker never deletes or mutates a `Task` outside the reliable-queue ACK path; DLQ depth exposed as `queue_depth{queue="dlq"}` metric | Platform engineer | Monthly |
...
| TR-003 | Metric | `queue_reaper_reclaimed_total` counter increments when the reaper returns timed-out inflight tasks | Worker process |
| TR-004 | Metric | `queue_depth{queue="dlq"}` gauge exposes dead-letter queue size; alert at documented threshold | Worker process |
