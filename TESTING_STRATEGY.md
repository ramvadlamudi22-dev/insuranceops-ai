...
- **Assertion.** For each event after the first, `event_hash` equals
  `sha256(audit_event_id || workflow_run_id || actor || event_type || canonical_json(event_payload) || occurred_at_iso || coalesce(prev_event_hash, b''))`.
  The first event has `prev_event_hash` = NULL and computes `event_hash` with
  empty bytes substituted for the absent prev_event_hash (i.e., `coalesce(NULL, b'')` = `b''`). The last event's
